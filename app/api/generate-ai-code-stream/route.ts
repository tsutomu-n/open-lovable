...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, context, isEdit = false } = body;
    ...
    console.log('[generate-ai-code-stream] - model:', requestedModel);
...
`;
        ...
        const isKimiGroq = model === 'moonshotai/kimi-k2';
        const resolvedModel = resolveLanguageModel(model);
        let actualModel = resolvedModel.actualModel;

        console.log(`[generate-ai-code-stream] Using provider: ${resolvedModel.viaGateway ? 'Gateway' : resolvedModel.provider}, model: ${actualModel}`);
          ...
          try {
            ...
            if (retryCount < maxRetries && isRetryableError) {
              ...
              if (isGroqServiceError && retryCount === maxRetries) {
                const fallbackModelId = catalog.models.find(
                  availableModel => availableModel.enabled && availableModel.id !== model
                )?.id;

                if (fallbackModelId) {
                  const fallbackModel = resolveLanguageModel(fallbackModelId);
                  console.log(`[generate-ai-code-stream] Groq service unavailable, falling back to ${fallbackModelId}`);
                  streamOptions.model = fallbackModel.model;
              ...
              if (isGoogle) {
                await sendProgress({ 
                  type: 'info', 
                  message: isUsingAIGateway
                    ? 'Tip: Make sure AI_GATEWAY_API_KEY is set correctly and the selected Google model is available in AI Gateway.'
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                const completionModel = resolveLanguageModel(model);
